...
def render_extras(strategy: str, extras: dict, *, nested: bool = False) -> None:
    """전략별 부가 정보. nested=True 이면 expander를 쓰지 않는다(중첩 금지)."""
    if not extras:
        return

    if strategy == "HyDE" and extras.get("hypothetical_doc"):
        if nested:
            st.markdown("**HyDE 가상 문서 (검색용, 답변 근거 아님)**")
    ...
    if strategy == "MultiQuery":
        ...
        if queries:
            if nested:
                st.markdown("**Multi-Query 재작성 질의**")
    ...
    if run_cmp:
        ...
        if not q:
            ...
            for i, name in enumerate(compare_strategies, 1):
                ...
                try:
                    rows.append(engine.run_strategy(name, q, with_extras=True))
                except Exception as e:
                    rows.append(
                        {
                            "strategy": name,
    ...
    if rows:
        ...
        for r in rows:
            with st.expander(f"{r['strategy']}  · 연도 {r['years_hit']}  · {r['total_time_s']}s", expanded=False):
                render_extras(r["strategy"], r.get("extras") or {}, nested=True)
                st.markdown("**출처**")
...
with tab_chat:
    ...
    token_rows = st.session_state.chat_metrics
    if len(token_rows) >= 2:
        token_df = pd.DataFrame(
            {
                "턴": list(range(1, len(token_rows) + 1)),
                "히스토리": [r["history_tokens"] for r in token_rows],
                "문서": [r["doc_tokens"] for r in token_rows],
            }
        )
        st.caption("히스토리 예산과 RAG 문서 예산을 분리해 표시합니다.")
        st.line_chart(token_df.set_index("턴"))
